...
import { IkigaiCategory, IkigaiData, IkigaiItem, DiagramZoneType, DiagramDisplayItems } from './types.ts'; // Added .ts
import { INITIAL_IKIGAI_DATA } from './constants.ts'; // Added .ts
import Header from './components/Header.tsx'; // Added .tsx
import Footer from './components/Footer.tsx'; // Added .tsx
import InputSection from './components/InputSection.tsx'; // Added .tsx
import IkigaiDiagram from './components/IkigaiDiagram.tsx'; // Added .tsx
import HoverItemsBlock from './components/HoverItemsBlock.tsx'; // Added .tsx
...
const App: React.FC = () => {
  ...
  const calculateIntersection = useCallback((data: IkigaiData, catsToIntersect: IkigaiCategory[]): IkigaiItem[] => {
    ...
    commonTexts.forEach(text => {
      ...
      for (const cat of catsToIntersect) {
        const foundItem = (data[cat] || []).find(item => item.text.trim().toLowerCase() === text);
        if (foundItem) {
          resultItems.push({ ...foundItem, id: `intersect-${cat}-${text.replace(/\s+/g, '-')}-${Math.random().toString(36).substring(7)}` }); 
          addedTexts.add(text);
          break; 
        }
      }
